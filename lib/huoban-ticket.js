  ...
  getTicket: function getTicket(args) {
    return new Promise(function (resolve, reject) {
      ...
      if (appType === 'enterprise') {
        huobanTicket.setEnterpriseTicket(args).then(function () {
          resolve(huobanTicket.ticket);
        });
      } else {
        huobanTicket.setTableTicket(args).then(function () {
          resolve(huobanTicket.ticket);
        });
      }
    });
  ...
  setEnterpriseTicket: function setEnterpriseTicket(args) {
    return new Promise(function (resolve, reject) {
      if (args.nocache || !huobanTicket.ticket) {
        ...
        huobanTicket.ticket = _jsCookie["default"].get('eTicket');
        resolve();
      }
    });
  },
  setTableTicket: function setTableTicket(args) {
    return new Promise(function (resolve, reject) {
      if (args.nocache || !huobanTicket.ticket) {
        ...
        huobanTicket.ticket = _jsCookie["default"].get('tTicket');
      }

      _jsCookie["default"].set('tTicket', huobanTicket.ticket);
